...
describe('caseInterceptor', () => {
  ...
  it('passes Named Query result columns through unchanged', done => {
    const raw = { resource: [{ nr_carcaca: 'KE120-0055820' }] };
    http.get('/api/v2/py_ptg/_query/acasala').subscribe((body: any) => {
      expect(body).toEqual(raw);
      done();
    });
    const req = httpMock.expectOne('/api/v2/py_ptg/_query/acasala');
    req.flush(raw, { headers: { 'Content-Type': 'application/json' } });
  });

  it('preserves Named Query parameter names in request bodies', done => {
    http
      .post('/api/v2/py_ptg/_query/by-vin', { vinCode: '9C6KG0460P0000001' })
      .subscribe(() => done());
    const req = httpMock.expectOne('/api/v2/py_ptg/_query/by-vin');
    expect(req.request.body).toEqual({ vinCode: '9C6KG0460P0000001' });
    req.flush({ resource: [] }, { headers: { 'Content-Type': 'application/json' } });
  });
