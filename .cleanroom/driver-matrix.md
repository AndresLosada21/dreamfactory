...
| **dual-driver Oracle** | `oci8 3.4.x` (ext-oci8) **ou** `pdo_oci` via `yajra/laravel-pdo-via-oci8 3.7.3` — ambos pinados em `Dockerfile:21` + `composer.json:134` | MIT (yajra) + PHP-3.01 | Provado `docker exec dreamfactory php -m | grep -E "oci8|pdo_oci"` + `extensions/df-oracle/src/ServiceProvider.php:23` | Baixo | **GO — dual-driver sem OTN** — `legal_signoff` OTN bypass via docs |
| **dual-driver SQL Server** | `sqlsrv 5.12.x` (ext) **ou** `pdo_sqlsrv 5.12.x` + `msodbcsql18` — `Dockerfile:31` `docker-php-ext-enable sqlsrv pdo_sqlsrv` | MIT + Microsoft EULA | Provado `php -m | grep pdo_sqlsrv` + `extensions/df-sqlsrv/src/Database/Schema/SqlServerSchema.php:15` | Médio | **GO — dual-driver mitiga EULA** |
| **dual-driver Informix** | `pdo_informix 1.3.7` **ou** `PDO_INFORMIX via CSDK` — `Dockerfile:24-30` `phpize && make && phpenmod pdo_informix` | Apache-2.0 + IBM ILA | Provado `php -m | grep pdo_informix` + `InformixConnector.php:11` falha segura | Médio | **GO spike / CONDICIONAL produção** |
