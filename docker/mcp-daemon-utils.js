...
export function createServer(serviceName, apiConfigs, sessionManager, disabledTools, customTools) {
    ...
    const instructions = [
        ...
        `   If it TIMES OUT on a large/busy database (row counts are the bottleneck), fall back to`,
        `   \`{prefix}_get_tables\` + \`{prefix}_get_table_schema\`, or a governed named query when one exists`,
        `   (e.g. \`py_ptg_data_model\` returns columns/PK/FK via information_schema in <1s).`,
        ...
        '',
        '## Resource Names & Discovery (IMPORTANT)',
        '- `tableName` must match the DreamFactory resource catalog name, NOT a raw SQL identifier.',
        '- If unsure of the exact name, call `all_find_table` or `{prefix}_get_tables` FIRST — never guess.',
        '- Oracle services: names are UPPERCASE. Objects in the service default schema are listed WITHOUT',
        '  schema prefix (e.g. `YM_WMS_VW_LPN_ERP`); objects in other schemas appear qualified (e.g. `SYS.ALL_OBJECTS`).',
        '- PostgreSQL services: use the schema-qualified name exactly as listed (e.g. `ynslf_ymda_pub_a3.vic_s_int_bprd_ctl`).',
        '- Views are exposed like tables and are queryable with `{prefix}_get_table_data`.',
        '- Named query SQL is raw SQL sent to the database: there you CAN use schema-qualified identifiers.',
        '',
        '## Named Queries (governed, read-only SQL)',
        '- Discover: `named_query_list_published` (executable ones) or `named_query_list_all` (incl. drafts).',
        '- Inspect: `named_query_get` returns the full definition incl. revisions, SQL and parameter declarations.',
        '- Execute: `named_query_run` with `service`, `name` and `params_json` — a JSON object STRING whose keys',
        '  match the declared parameters, e.g. params_json = {"cma":"KE1200055820"}. Types and required flags',
        '  are validated server-side; optional parameters may be omitted when a default is declared.',
        '- Lifecycle: `named_query_create` makes an INACTIVE draft → `named_query_publish` makes it executable.',
        '  To change SQL later: `named_query_revise` (new revision) then `named_query_publish` that revision.',
        '- Optimistic locking via `lock_version`: on 409 Conflict, re-read with `named_query_get` and retry.',
        '- SQL is enforced READ-ONLY server-side: must start with SELECT or WITH; no DML, no semicolons.',
        '- Running an unpublished query returns 404. Large result sets: prefer queries with LIMIT declared.',
        '',
